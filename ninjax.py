...
def FromFlax(ctor, rng_names=None):
  class FlaxModule(Module):
    def __init__(self, *args, **kwargs):
      self.module = ctor(*args, **kwargs)
      if rng_names is None:
        self._rng_names_init = ["params"]
        self._rng_names_apply = []
      elif isinstance(rng_names, list):
        assert "params" not in rng_names, "params key for rng is reserved for flax module parameter initialization. Please use another rng stream name."
        self._rng_names_init = rng_names + ["params"]
        self._rng_names_apply = rng_names.copy()
      else:
        raise ValueError(rng_names)

    def __call__(self, *args, **kwargs):
      seed_init = {k: seed() if creating() else None for k in self._rng_names_init}
      state = self.get('flax', self.module.init, seed_init, *args, **kwargs)
      seed_apply = {k: seed() for k in self._rng_names_apply}
      return self.module.apply(state, *args, rngs=seed_apply, **kwargs)
  return FlaxModule
